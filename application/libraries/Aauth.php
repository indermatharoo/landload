...
class Aauth {
    ...
    public function login($email, $pass, $remember = FALSE) {
        ...
        if ($query->num_rows() > 0 and $this->config_vars['ddos_protection'] and ! $this->update_login_attempts($row->email) and false) {
...
//            $this->error($this->CI->lang->line('email_taken'));             
        ...
        if ($this->user_exsist_by_name($name)) {
            $this->CI->session->set_flashdata('ERROR', 'name_taken');
//            $this->error($this->CI->lang->line('name_taken'));
            $valid = false;
        }

        if (!valid_email($email)) {
            $this->CI->session->set_flashdata('ERROR', 'email_invalid');
...
//            $this->error($this->CI->lang->line('pass_invalid'));
        ...
        if ($name != '' and ! ctype_alnum(str_replace($this->config_vars['valid_chars'], '', $name))) {
            $this->CI->session->set_flashdata('ERROR', 'name_invalid');
...
die();
     ...
     */
    public function get_user($user_id = FALSE, $select_fld = null) {

        if ($user_id == FALSE)
            $user_id = $this->CI->session->userdata('id');
        if ($select_fld)
            $this->CI->db->select($select_fld);
        $query = $this->CI->db->where('id', $user_id);
        $query = $this->CI->db->get($this->config_vars['users']);
    ...
    public function get_user_groups($user_id = false) {
        ...
        return $query = $this->CI->db->get()->row_array();
    }
     ...
     */
    public function list_groups($inArray = false) {
    ...
    public function list_group_key_pair_form() {
        $internalArr = $this->list_groups(true);
        $retAr = array();
        foreach ($internalArr as $interArrK => $interArrV) {
            $retAr[$interArrV['id']] = $interArrV['name'];
        }
        return $retAr;
    }
    ...
    private $super_franchise = 1;
    private $franchisor = 2;
    private $franchisee = 3;
    private $user = 5;
    private $customer = 6;

    public function getUsersIdRole() {
        return array(
            $this->user => 'USER',
            $this->customer => 'CUSTOMER',
            $this->super_franchise => 'SUPER',
            $this->franchisor => 'FRANCHISOR',
            $this->franchisee => 'FRANCHISEE',
        );
    }

    public function check_group($group_id, $user_id = null) {
        if (!$user_id)
            $user_id = $this->CI->session->userdata('id');
        $user_id = $this->CI->session->userdata('id');
        $this->CI->db->select('*');
        $this->CI->db->from($this->config_vars['user_to_group']);
        $this->CI->db->join($this->config_vars['groups'], "id = group_id and group_id = " . $group_id);
        $this->CI->db->where('user_id', $user_id);
    ...
    /*
     * return bool true if current login use is user
     */

    public function isUser($id = false) {
        if ($id)
            return $this->user_group['user'] == self::getUserGroup($id) && (!self::isFranshisor(self::get_user($id)->pid));
        else
            return $this->CI->session->userdata('isUser');
    }

    /*
     * return bool true if current login use is customer
     */

    public function isCustomer($id = null) {
        if ($id) {
            return $this->user_group['customer'] == self::getUserGroup($id);
        } else {
