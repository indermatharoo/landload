...
class HeadAssets {
	...
	function renderHead() {
		...
		//Minified
		$DWS_MIN_CSS_ARR = array_unique($this->h_css_resources_min);
		...
		//Normal CSS Files
		$DWS_CSS_ARR = array_unique($this->h_css_resources);
		...
		//Minified JS
		$DWS_MIN_JS_ARR = array_unique($this->h_js_resources_min);
		if (count($DWS_MIN_JS_ARR) > 0) {
			$js = join(",", $DWS_MIN_JS_ARR);
			echo '<script type="text/javascript" src="' . $base_url . 'min/?f=' . $js . '"></script>
			';
		}

		//Normal JS Files
		$DWS_JS_ARR = array_unique($this->h_js_resources);
		foreach ($DWS_JS_ARR as $js) {
			echo '<script type="text/javascript" src="' . $base_url . $js . '"></script>
				';
		}

		//Inline JS Scripts
		$DWS_JS_ARR = $this->h_js_scripts;
		foreach ($DWS_JS_ARR as $js) {
			echo "$js\r\n" ;
		}
	}
	...
	function renderFooter() {
		...
		//Minified JS
		$DWS_MIN_JS_ARR = array_unique($this->f_js_resources_min);
		if (count($DWS_MIN_JS_ARR) > 0) {
			$js = join(",", $DWS_MIN_JS_ARR);
			echo '<script type="text/javascript" src="' . $base_url . 'min/?f=' . $js . '"></script>
			';
		...
		//Normal JS Files
		$DWS_JS_ARR = array_unique($this->f_js_resources);
		foreach ($DWS_JS_ARR as $js) {
			echo '<script type="text/javascript" src="' . $base_url . $js . '"></script>
				';
		}
