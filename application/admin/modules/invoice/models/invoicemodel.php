...
//    }
    ...
    function getAll() {
        ...
        elseif ($this->aauth->isUser()):
            $this->db->where('company_id', curUsrPid());
        endif;
    ...
    function getQuarterlyDetailInvoice() {
        $this->db->select("*")
                ->from("invoice_new")
    ...
    function getHalfYearlyDetailInvoice() {
        $this->db->select("*")
                ->from("invoice_new")
...
//        $data['terms'] = $this->input->post('terms', true);
    ...
    function countAllWeekly($ids = array()) {
        $this->db->select("*");
        $this->db->from("invoice_new");
        $this->db->where("invoice_type", 'W');
        if (count($ids)) {
            $this->db->where_in('company_id', $ids);
        }
        return $this->db->count_all_results();
    ...
    function countAllMonthly($ids = array()) {
        $this->db->select("*");
        $this->db->from("invoice_new");
        $this->db->where("invoice_type", 'M');
        if (count($ids)):
            $this->db->where_in('company_id', $ids);
        endif;
        return $this->db->count_all_results();
