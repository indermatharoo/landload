...
<p align="center">&nbsp;</p>
<p align="center"><img src="<?= site_url1(base_url()); ?>images/activityanimation.gif" alt="Redirecting" width="70" height="7" /></p>
<?php
//e($order);
?>
...
<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post" name="payment_form" id="payment_form">
    ...
    <input type="hidden" name="quantity" value="<?php echo arrIndex($order, 'ctn') ?>">
    <input type="hidden" name="currency_code" value="<?= DWS_CURRENCY_CODE; ?>">
    <input type="hidden" name="custom" value="<?= $order['id'] . ':' . curUsrId(); ?>">
    <input type="hidden" name="return" value="<?php echo base_url() . "customer/checkout/success/{$order['unique_id']}"; ?>">
    <input type="hidden" name="cancel_return" value="<?php echo base_url() . "customer/checkout/failed/{$order['unique_id']}"; ?>">
    <input type="hidden" name="notify_url" value="<?php echo base_url() . "checkout/payment/index"; ?>">
</form>
<p align="center"><img src="<?= site_url1(base_url()); ?>images/horizontal_solution_PPeCheck.png" width="166" height="42" /></p>
<script>
