...
class Junkcarblock extends Admin_Controller {
    ...
    function valid_block($str) {
        ...
        $this->db->from('cash_for_junk_cars');
    ...
    function index($pid = 0, $offset = 0) {
        $this->load->model('Junkcarblockmodel');
        $this->load->model('Pagemodel');
        $this->load->library('form_validation');
        $this->load->helper('form');
        $this->load->library('pagination');

        if (!$this->checkAccess('MANAGE_PAGE_BOLCKS')) {
        ...
        //Setup pagination
        $perpage = 2000;
        $config['base_url'] = base_url() . "cms/junkcarblock/index/$pid/";
        $config['uri_segment'] = 5;
        $config['total_rows'] = $this->Junkcarblockmodel->countAll($pid);
        $config['per_page'] = $perpage;
        $this->pagination->initialize($config);

        //list all block
        $block = array();
        $block = $this->Junkcarblockmodel->listAll($pid, $offset, $perpage);
        ...
        $page = array();
        $page['content'] = $this->load->view('junkcar/block-index', $inner, TRUE);
    ...
    function add($bid = false) {
        ...
        if (!$this->checkAccess('ADD_HOMEPAGE_BOLCK')) {
            $this->utility->accessDenied();
            return;
        }
        ...
        //Get page Detail
        $pages = array();
        $pages = $this->Junkcarblockmodel->getPageDetail($bid);
		if (!$pages) {
            $this->utility->show404();
            return;
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $page = array();
            $page['content'] = $this->load->view('junkcar/block-add', $inner, TRUE);
            $this->load->view('shell-overlay', $page);
        } else {
            $this->Junkcarblockmodel->insertRecord($pages);

            $this->session->set_flashdata('SUCCESS', 'block_added');

            redirect("cms/junkcarblock/index/$bid", "location");
            exit();
        }
    }
    ...
    function edit($bid = false) {
        ...
        if (!$this->checkAccess('EDIT_HOMEPAGE_BOLCK')) {
            $this->utility->accessDenied();
            return;
        }
        ...
        $block = array();
        $block = $this->Junkcarblockmodel->getDetails($bid);
        if (!$block) {
            $this->utility->show404();
            return;
        }
        ...
        //Get page Detail
        $pages = array();
        $pages = $this->Junkcarblockmodel->getPageDetail($block['page_id']);
        if (!$pages) {
            $this->utility->show404();
            return;
        }
    ...
    function delete($bid = false) {
        ...
        if (!$this->checkAccess('DELETE_HOMEPAGE_BOLCK')) {
            $this->utility->accessDenied();
            return;
        }
