...
class Payment extends Cms_Controller {
    ...
    function index() {
        ...
        if ($_POST['receiver_email'] != DWS_MERCHANT_EMAIL) {
            if ($this->debug)
                log_message($this->log_level, "Receiver Email ({$_POST['receiver_email']}) didn't match client's PayPal ID " . DWS_MERCHANT_EMAIL);
            return;
        }
