jQuery(document).ready(function($) {
	...
	$('.category_margin').click(function() {
		...
		var cp_id = $('input[name=company_id]').val();
		...
		$("#warning-message").dialog({
			...
			buttons: {
				"Yes": function() {
					$.post('company/price/updatecategorymargin', {category_margin: cat_margin, category_id: cat_id, company_id: cp_id}, function(data) {
						if (data.status == 1) {
							window.location.href = 'company/price/index/' + cp_id;
						}
					}, 'json');
				},
				Cancel: function() {
